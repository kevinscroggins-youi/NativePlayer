...
CYIRemoteLogger.version = "1.1.11"; // should match the version on the RESTful API server, but there is backwards and forwards compatibility as well, for now

CYIRemoteLogger.regExpFlags = Object.freeze({
    global: "g",
    multiline: "m",
    ignoreCase: "i",
    sticky: "y",
...
CYIRemoteLogger.initialize = function initialize() {
	...
	if(CYIRemoteLogger.legacy !== null) {
		CYIRemoteLogger.originalPrintLogFunction = CYIRemoteLogger.legacy ? application.printLog : (typeof CYILogger === "undefined" ? undefined : CYILogger.log);
		...
		CYIRemoteLogger.originalHookConsoleLoggingFunction = CYIRemoteLogger.legacy ? application.hookConsoleLogging : (typeof CYILogger === "undefined" ? undefined : CYILogger.hookConsoleLogging);
		CYIRemoteLogger.originalUnhookConsoleLoggingFunction = CYIRemoteLogger.legacy ? application.unhookConsoleLogging : (typeof CYILogger === "undefined" ? undefined : CYILogger.unhookConsoleLogging);
	}
	...
	if((CYIRemoteLogger.originalHookConsoleLoggingFunction instanceof Function) && (CYIRemoteLogger.originalUnhookConsoleLoggingFunction instanceof Function)) {
		function hookConsoleLogging() {
			...
			if(CYIRemoteLogger.legacy) {
				if(typeof application !== "undefined") {
					CYIRemoteLogger.originalHookConsoleLoggingFunction.apply(application, arguments);
				}
			}
			else if(typeof CYILogger !== "undefined") {
				CYIRemoteLogger.originalHookConsoleLoggingFunction.apply(CYILogger, arguments);
			}
		...
		function unhookConsoleLogging() {
			...
			if(CYIRemoteLogger.legacy) {
				if(typeof application !== "undefined") {
					CYIRemoteLogger.originalUnhookConsoleLoggingFunction.apply(application, arguments);
				}
			}
			else if(typeof CYILogger !== "undefined") {
				CYIRemoteLogger.originalUnhookConsoleLoggingFunction.apply(CYILogger, arguments);
			}
		}
...
CYIRemoteLogger.isInitialized = function isInitialized() {
	return CYIRemoteLogger.initialized;
};

CYIRemoteLogger.onLogSinkInitialized = function onLogSinkInitialized() {
	CYIRemoteLogger.sendDebugRemoteLog("Log sink initialized, unhooking print log function.");
	...
	if(CYIRemoteLogger.shouldHookPrintLogFunction) {
		...
		function printLog() {
			...
			if(arguments.length !== 0 && typeof arguments[0] === "string") {
				...
				if(logMessage.indexOf("/Tizen", 2) !== -1) {
					doNotRemoteLog = true;
				}
				else if(logMessage.indexOf("I/") !== -1) {
					logType = "info";
				}
				else if(logMessage.indexOf("W/") !== -1) {
					logType = "warning";
				}
				else if(logMessage.indexOf("E/") !== -1 || logMessage.indexOf("F/") !== -1) {
					logType = "error";
				}
			}
			...
			if(CYIRemoteLogger.originalPrintLogFunction instanceof Function) {
				if(CYIRemoteLogger.originalPrintLogFunctionRequiresTypeArgument) {
					...
					if(CYIRemoteLogger.legacy) {
						if(typeof application !== "undefined") {
							CYIRemoteLogger.originalPrintLogFunction.apply(application, arguments);
						}
					}
					else if(typeof CYILogger !== "undefined") {
						CYIRemoteLogger.originalPrintLogFunction.apply(CYILogger, arguments);
					}
				}
			}
...
CYIRemoteLogger.toString = function toString(value) {
	if(value === undefined) {
		...
		for(var flag in CYIRemoteLogger.regExpFlags) {
			if(value[flag]) {
				flags += CYIRemoteLogger.regExpFlags[flag];
			}
		}
